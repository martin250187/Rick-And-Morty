...
const Card = (props) => {
  return (
    <div className={styled.containerCard}>
      <button className={styled.button} onClick={props.onClose}>X</button>
      <h2 className={styled.box}>{props.id}</h2>
      ...
      <img className={styled.img} src={props.image} alt="No se encuentra la imágen!" />
    </div>
  );
};
